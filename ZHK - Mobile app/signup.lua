...
local function handleResponse( event )
    ...
    if ( event.isError)  then
      ...
      if myNewData=="Register done!\n\n\n" then
				hideMain()
      elseif myNewData=="Email exsist\n\n\n" then
		  	showWarnin("Эта почта уже зарегистрирована")
		  else
		  	-- print(myNewData)
...
local function submitFunc(event)
	...
	elseif allows then
		...
		time = time.day.."."..os.date("%m",os.time()).."."..time.year
		print("REQUEST")
		q.getConnection("","register.php", handleResponse,{
			email = email,
			hasedPassword = q.hashText(pass),
			date = time,
			name = name
		})
