...
local function createButton(group,label,y, name)
	local submitButton = display.newRoundedRect(group, 50, y, q.fullw-50*2, 110, 12)
...
function scene:create( event )
	...
	local labelDiscription = display.newText( {
		parent = mainGroup,
		text = "Сервера сейчас не доступны",
		x = 50,
		y = 440+100+80,
		font = "ubuntu_r.ttf",
		fontSize = 16*2,
	...
	local labelWannaTry = display.newText( {
		parent = mainGroup,
		text = "Желаете протестировать локально?",
		x = q.cx,
		y = q.fullh-50-130*3-80,
		font = "ubuntu_r.ttf",
		fontSize = 18*2,
		} )
	labelWannaTry.fill = c.white



	local user = createButton(mainGroup, "ПОЛЬЗОВАТЕЛЬ",q.fullh-50-130*2,user)
	local worker = createButton(mainGroup, "СОТРУДНИК",q.fullh-50-130,worker)
	local admin = createButton(mainGroup, "АДМИНИСТРАТОР",q.fullh-50,admin)
  local workerStatus = json.decode(q.getConnection("static"))

	local userData = [[{"id":"19","email":"admin@gmail.com","plan":"BASIC","name":"Терёхин Матвей Андреевич","sity":"Якутск","lic":"admin","phonenumber":"","password":"12345678","signupdate":"3 May 2022","working":"0"}]]
  ...
  data.sity="Якутск"
    ...
    data.name="Софронов Александр Иннокентьевич"
    data.working=workerStatus["1"][1]
		q.saveLogin(data)
		composer.gotoScene( "menu" )
		composer.removeScene( "signtest" )
	end )
	worker:addEventListener( "tap", function()
    data.id="2"
    data.lic="worker"
    data.email="worker@gmail.com"
    data.name="Филиппов Егор Донатович"
    data.working=workerStatus["1"][2]
		q.saveLogin(data)
		composer.gotoScene( "menu" )
		composer.removeScene( "signtest" )
	end )
	admin:addEventListener( "tap", function()
    data.id="3"
    data.lic="admin"
    data.email="admin@gmail.com"
    data.name="Ершов Яков Лаврентьевич"
    data.working=workerStatus["1"][3]
		q.saveLogin(data)
		composer.gotoScene( "menu" )
		composer.removeScene( "signtest" )
	end )
