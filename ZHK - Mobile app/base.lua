...
local baseScriptsUrl = "http://127.0.0.1/dashboard/profine/"
...
local base = {
  ...
  getConnection = function( path, url, func,params )
    if isLocalBase then
  ...
  event = {
    ...
    group = { 
      ...
      on = function(groupName, enable)
        if events[groupName]==nil then error("Enable event group: bad argument #1 (group ".. groupName .." not exist)") end
